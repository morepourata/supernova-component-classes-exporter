...
export function styleOutputFile(tokens: Array<Token>, tokenGroups: Array<TokenGroup>, themePath: string = '', theme?: TokenTheme): OutputTextFile | null {
    ...
    // Add component classes (@layer components for Components collection tokens)
    content += generateComponentContent(processedTokens, tokenGroups)
...
export function generateStyleFiles(tokens: Array<Token>, tokenGroups: Array<TokenGroup>, themePath: string = '', theme?: TokenTheme): Array<OutputTextFile> {
    ...
    return Array.from(tokensByType.entries()).map(([type, tokensOfType]) => {
        ...
        // Add component classes (runs for all types since we filter internally)
        content += generateComponentContent(tokensOfType, tokenGroups)
...
export function generateComponentContent(tokens: Array<Token>, tokenGroups: Array<TokenGroup>): string {
  ...
  for (const token of componentTokens) {
    ...
    const variantMap = classMap.get(className)!
    if (!variantMap.has(variantName)) variantMap.set(variantName, [])
    variantMap.get(variantName)!.push({ prop: cssProperty, varName: cssVarName })
  }
  ...
  for (const [className, variantMap] of classMap) {
    ...
    if (baseDecls && baseDecls.length > 0) {
      output += `  .${className} {\n`
      for (const { prop, varName } of baseDecls) {
        output += `    ${prop}: var(${varName});\n`
      }
      output += `  }\n`
    }
    for (const [variantName, decls] of variantMap) {
      ...
      for (const { prop, varName } of decls) {
        output += `    ${prop}: var(${varName});\n`
      }
      output += `  }\n`
    }
